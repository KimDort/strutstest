...
public class ProjectJoinDAO {
	...
	public List<ProjectJoin> list(Connection conn, SearchCriteria cri)throws SQLException{
		...
		String sql="SELECT "
				+"		JOIN_NO, PROJECT_NO, MEMBER_NO, MEMBER_ISNEW, JOIN_POSITION, JOIN_IN, JOIN_OUT, PROJECT_NAME, EMPLOYEE, R "
				+"	FROM "
				+"		(SELECT "
				+"			JOIN_NO, PROJECT_NO, MEMBER_NO, MEMBER_ISNEW, JOIN_POSITION, JOIN_IN, JOIN_OUT, PROJECT_NAME, EMPLOYEE, ROWNUM AS R "
				+"		FROM "
				+"			("
				...
				+ "				(SELECT MEMBER_ISNEW FROM EMPLOYEE WHERE MEMBER_NO=PROJECT_JOIN.MEMBER_NO)AS MEMBER_ISNEW "
