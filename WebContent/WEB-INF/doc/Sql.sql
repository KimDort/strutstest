...
SELECT LISTAGG(MEMBER_HAVESKILL, ',') WITHIN GROUP(ORDER BY MEMBER_HAVESKILL)  AS MEMBER_HAVESKILL FROM EMPLOYEE;
	
select to_char(project_start, 'YYYY-MM-DD'), project_end from project

SELECT PROJECT_NAME 
FROM PROJECT 
WHERE PROJECT_START >= CASE WHEN '2017-01-01' IS NULL THEN TO_CHAR(PROJECT_START, 'YYYY-MM-DD') ELSE TO_CHAR(TO_DATE('2017-01-01'),'YYYY-MM-DD') END
AND PROJECT_END <= CASE WHEN '2017-12-30' IS NULL THEN TO_CHAR(PROJECT_END, 'YYYY-MM-DD') ELSE TO_CHAR(TO_DATE('2017-12-31'), 'YYYY-MM-DD') END
